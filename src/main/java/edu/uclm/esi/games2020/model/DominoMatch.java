...
public class DominoMatch extends Match {
	...
	public boolean verifyPlay(WebSocketSession session, int number1, int number2, boolean posicionTablero) {
		final String colocaFicha = "\nEl jugador ha colocado la ficha: ";
		if (session != this.turn) {
			return false;
		}
		...
		for(int i=0; i < this.fichasJugadores.size()&&!existeFicha; i++) {
		...
		if(!tablero.isEmpty()) {
			String estadoTablero = "\nEl estado del tablero de tamaño " +this.tablero.size()+ "es antes de colocar:";
			log.info("\n");
			log.info(estadoTablero);

			for (FichaDomino elem : this.tablero) {
				String numerosFichaTablero = "["+elem.getNumber1()+" | "+elem.getNumber2()+"] ";
				log.info(numerosFichaTablero);
			}
			log.info("\n");
			...
			if(posicionTablero) {
				...
				assert fichaIzq != null;
				String estadoFichaTableroIzq = "\nLa ficha del tablero a la izquierda es: "+fichaIzq.getNumber1()+" | "+fichaIzq.getNumber2();
				log.info("\n");
				log.info(estadoFichaTableroIzq);
				if(fichaIzq.getNumber1()==this.fichaColocada.getNumber1()) {
					log.info("\nEntro en correcta izq rotando");
					String jugadaRealizada = colocaFicha+this.fichaColocada.getNumber1()+" | "+this.fichaColocada.getNumber2();
					log.info(jugadaRealizada);
					...
					log.info("\nEntro en correcta izq sin rotar");
					String jugadaRealizada = colocaFicha+this.fichaColocada.getNumber1()+" | "+this.fichaColocada.getNumber2();
					log.info(jugadaRealizada);
				...
				String estadoFichaTableroDer = "\nLa ficha del tablero a la derecha es: "+fichaDer.getNumber1()+" | "+fichaDer.getNumber2();
				log.info(estadoFichaTableroDer);
				if(fichaDer.getNumber2()==this.fichaColocada.getNumber1()) {
					log.info("\nEntro en correcta derecha sin rotar");
					String jugadaRealizada = colocaFicha+this.fichaColocada.getNumber1()+" | "+this.fichaColocada.getNumber2();
					log.info(jugadaRealizada);
					colocacionCorrecta = true;
					...
					log.info("\nEntro en correcta derecha rotando");
					String jugadaRealizada = colocaFicha+this.fichaColocada.getNumber1()+" | "+this.fichaColocada.getNumber2();
					log.info(jugadaRealizada);
			...
			String estadoTablero = "\nEl tablero esta vacío y coloca ficha: "+this.fichaColocada.getNumber1()+" | "+this.fichaColocada.getNumber2();
			log.info(estadoTablero);
			tablero.addFirst(this.fichaColocada);
		}
