...
function getFichaSelected(){
    let e = document.getElementById("fichas").value;
...
function changeColorCell(id, color){
    let celda = document.getElementById(id);
    celda.style.background = color;
}

function initDivMouseOver(){
    let but1 = obtenerElement("addIzq");
    let but2 = obtenerElement("addDer");
    but1.onmouseover = function () {
        changeColorCell('box' + vm.contBefore.toString(), '#d4d4d4');
    };

    but1.onmouseout = function () {
        changeColorCell('box' + vm.contBefore.toString(), '#ffffff');
    };

    but2.onmouseover = function () {
        changeColorCell('box' + vm.contAfter.toString(), '#d4d4d4');
    };
    but2.onmouseout = function () {
        changeColorCell('box' + vm.contAfter.toString(), '#ffffff');
    };
